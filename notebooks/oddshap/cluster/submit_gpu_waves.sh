...
#
#   bash reproduction/cluster/submit_gpu_waves.sh v522_merged          # all experiments
...
#   nohup bash reproduction/cluster/submit_gpu_waves.sh v522_merged > logs/gpu_waves.out 2>&1 &
set -uo pipefail
cd "$HOME/oddshap_reproduction"
mkdir -p logs reproduction/data
...
for VF in $GPU_VFS; do
  for INST in $(seq 0 $((N_INST - 1))); do
    ...
    sbatch -J "gpu1_${VF}_${INST}_${VARIANT}" -o "logs/gpu1_${VF}_${INST}_${VARIANT}.out" \
      ...
      reproduction/cluster/gpu_one.sbatch >/dev/null \
