...
which OddSHAP variant produced it. Data is read from the committed ``data/`` directory that the cluster scripts write
into, so every notebook re-renders offline without re-running the experiments.
"""
...
def paper_dir() -> Path:
    return _NB_DIR / "paper_reference"
