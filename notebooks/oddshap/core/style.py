...
def estimator_style(name: str, *, emphasize: bool = None) -> dict:
    ...
    """
    base = dict(
        _ESTIMATOR_STYLE.get(name, dict(color=OKABE_ITO["black"], marker="o", linestyle="-"))
    )
...
def vf_style(vf: str) -> dict:
    ...
    return dict(
        color=_VF_COLORS[i % len(_VF_COLORS)],
        marker=_VF_MARKERS[i % len(_VF_MARKERS)],
        linestyle=_VF_LINES[i % len(_VF_LINES)],
        lw=1.8,
        ms=4,
    )
...
PAPER_COLOR = {
    "MSR": "#D0D0D0",
    "SVARM": "#8C8C8C",
    "PermutationSampling": "#555555",
    "PermSamp": "#555555",
    "ProxyLGBM": "#B0BEC5",
    "RegressionMSR": "#3F51B5",
    "3-PolySHAP": "#7CB342",
    "LeverageSHAP": "#00968A",
    "OddSHAP": "#E64A19",
...
PAPER_VF_ALIAS = {"realestate": "estate", "corrgroups60": "cg60", "independentlinear60": "il60"}
