...
\begin{table}[H]
    \centering
    \caption[\oddshap{} rank per value function, and the reproduced Table~1]{\oddshap{}'s rank on
        each value function (top), and the paper's Table~1 reproduced (bottom). Rank~1 on 7 of 8,
        both variants; the exception is the smallest $d$, where the paper also expects parity
        rather than a win. The average is over the six baselines listed, not the paper's larger
        pool.}
    ...
    \begin{tabular}{@{}lcccccccc@{}}
        ...
        \multicolumn{9}{@{}l}{\textit{Average rank over the eight value functions}} \\
        & \oddshap{} & kADDSHAP & RegMSR & PermSamp & KernelSHAP & SVARM & MSR & \\
        & \textbf{1.125} & 2.750 & 3.250 & 4.125 & 4.250 & 5.625 & 6.875 & \\
        \bottomrule
    \end{tabular}
\end{table}
...
% The bar chart that stood here plotted seven numbers, which Table~\ref{tab:oddshap:ranks} now
% carries in three lines. It was also a table rendered as an image: unselectable, and set in a
% different font from the rest of the report.
