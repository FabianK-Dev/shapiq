...
\begin{configtable}{Configuration of the \oddshap{} reproduction, following
...
\oddshap{} attains rank~1 on 7 of the 8 value functions, for an average rank of 1.125
(Figure~\ref{fig:oddshap:rank}). The result holds identically for both variants: PR~\#522 as
merged and the PR~\#560 follow-up give the same rank on every value function. The single
exception is DistilBERT ($d = 14$), where RegressionMSR takes rank~1 and \oddshap{} rank~2 ---
which is what the paper itself reports for its low-dimensional value functions, where it states
that \oddshap{} performs on par with RegressionMSR rather than ahead of it. The ordering the
paper claims therefore reproduces, including the exception the paper predicts.

The accuracy advantage appears with budget as the paper describes, and the reproduced curves
track the published ones over three orders of magnitude
(Figure~\ref{fig:oddshap:fig2}). The interaction ablation reproduces as well, including its
non-monotonicity: adding odd interactions helps up to a point and then hurts
(Figure~\ref{fig:oddshap:fig4}).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.86\textwidth]{figures/oddshap-rank-table.png}
    \caption[Average rank across the eight value functions]{%
        \textbf{Average rank across the eight value functions} (the paper's Table~1),
        reproduced for PR~\#522. Lower is better; rank~1 means a method had the lowest
        median MSE on a value function. \oddshap{} (highlighted) averages 1.125, i.e.\
        rank~1 on seven of the eight and rank~2 on the eighth.
        \emph{This average is over the six baselines shown and is therefore not comparable
        with the paper's 1.50, which is taken over a larger pool} --- see the discussion
        below. The grey banner beneath every figure in this chapter records the setup the
        figure was produced under, so no panel has to be read out of context.}
...
\begin{figure}[H]
    ...
    \caption[MSE against budget, ours against the paper's]{%
        \textbf{MSE against budget on Cancer ($d=30$)}, the reproduction of the paper's
        Figure~2. Both axes are logarithmic; each curve is the median over the 30 explained
        predictions and the band is the interquartile range across them.
        \textbf{(1)} our reproduction, run against the six baselines \shapiq{} provides.
        \textbf{(2)} the paper's own medians, redrawn from its Figure~2 --- note that the
        method set differs from panel~(1), because the paper compares against
        \leverageshap{}, \polyshap{} and Proxy~(LGBM), which panel~(1) does not include.
        The two panels are therefore not a like-for-like overlay, which is why panel
        \textbf{(3)} isolates \oddshap{} alone: ours (solid) against the paper's digitised
        median (dashed). The curves track each other over three orders of magnitude and
        both fall monotonically with budget, which is the claim being tested. The paper's
        curve is digitised from the published figure, so it carries the resolution of that
        source rather than author-provided values.}
...
\begin{figure}[H]
    ...
    \caption[The interaction ablation]{%
        \textbf{The interaction ablation} (the paper's Figure~4) at a fixed budget
        $m = 10{,}000$, for seven value functions --- Estate is excluded, as the paper
        excludes it for being an outlier. The $y$-axis is the MSE \emph{ratio} against the
        interaction-free baseline, so $1.0$ (the horizontal line) means adding odd
        interactions changed nothing, below~$1$ means it helped, above~$1$ means it hurt.
        The $x$-axis is the number of odd interactions admitted to the support, which the
        candidate budget $\lceil m/\eta \rceil$ fixes --- hence the $\eta$ values annotated
        beneath it, running from $\eta = 50$ (few interactions) to $\eta = 2$ (many).
        \textbf{(1)} ours, \textbf{(2)} the paper's, redrawn.
        The shape the paper predicts reproduces on every value function: the ratio drops as
        interactions are added, bottoms out around $\eta = 10$ to $\eta = 5$, and then rises
        \emph{above} 1 at $\eta = 2$, where the support is large enough to overfit and the
        estimate becomes worse than using no interactions at all. This non-monotonicity is
        the paper's argument for capping the candidate budget rather than enumerating.
        The depth of the trough differs between the panels --- ours reaches further below~1
        on Cancer than the paper's does --- so the reproduction supports the shape of the
        claim rather than its exact magnitude.}
...
and DistilBERT 1{,}591, rather than the paper's sweep to $\min(2^d, 20\,000)$, because the
rented GPU time ran out before the high-budget points finished. Both are low-dimensional
($d = 16$, $d = 14$) --- the regime in which the paper itself reports convergence between the
methods --- and the $\eta$ ablation is complete for both. The high-budget tail for these two is
nevertheless not confirmed here. Separately, the reference curves in
