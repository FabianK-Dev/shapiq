...
if r1 and r2:
    ...
    ax.bar(x - 0.2, [r1.get(e, np.nan) for e in ests], 0.4, label=R.VARIANT_LABEL[V1],
           color=R.variant_style(V1)["color"])
    ax.bar(x + 0.2, [r2.get(e, np.nan) for e in ests], 0.4, label=R.VARIANT_LABEL[V2],
           color=R.variant_style(V2)["color"], hatch="//")
    ax.set_xticks(x); ax.set_xticklabels(ests, rotation=30, ha="right", fontsize=8)
    ax.set_ylabel("average rank (1 = best)")
    ax.set_title(f"Average rank — PR #522 vs PR #560 ({len(used1)} value functions)")
    ax.legend(fontsize=8)
    R.add_banner(fig, "identical bars ⇒ #560 does not change the paper-scale ranking  |  "
...
for vf in ALL_VFS:
    t1 = R.load_table1(vf, V1); t2 = R.load_table1(vf, V2)
    if not (t1 and t2):
        continue
...
for vf in ["realestate", "cancer", "nhanes"]:
    c1 = R.load_fig2(vf, V1); c2 = R.load_fig2(vf, V2)
    if not (c1 and c2):
        continue
    ...
    for cv, variant, lab in [(c1, V1, "PR #522 (ours)"), (c2, V2, "PR #560 (author)")]:
        ...
        if pts:
            ax.plot([b for b, _ in pts], np.clip([v[0] for _, v in pts], 1e-32, None),
                    label=f"OddSHAP {lab}", **R.variant_style(variant))
    ...
    ax.set_xlabel("budget m"); ax.set_ylabel("Shapley MSE")
    ax.set_title(R.fig_title("Figure 2 — #522 vs #560 (OddSHAP)", vf, "library",
                             "identical ≥ n·η"))
